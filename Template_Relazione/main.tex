...
Parallelamente, per lo sviluppo dell'ambiente 3D e della visualizzazione in tempo reale, si è mantenuto \textbf{Unity 6} (versione 6000.0.26f1)su Windows, supportato da \textbf{Unity Hub} (versione 3.10.0) che permette una gestione semplificata dei vari progetti Unity. La scelta di utilizzare Windows come sistema operativo per Unity è motivata dalla sua eccellente compatibilità con gli strumenti grafici e i driver necessari per un'interfaccia utente fluida e performante, in quanto Unity è un software nativo Windows. La comunicazione tra l'ambiente robotico gestito tramite Ubuntu e quello grafico su Windows è resa possibile attraverso il ROS-TCP Connector, che funge da ponte tra le due piattaforme, garantendo lo scambio efficiente dei dati.
...
\begin{itemize}
    \item \textbf{Publisher Unity}: script in C\# che cattura ad intervalli regolari le texture della scena virtuale, estrae separatamente i dati RGB e quelli di profondità e li pubblica su due topic ROS2 distinti.
...
\begin{lstlisting}[caption=Script base di acquisizione immagini in C\#, label=lst:base, lineskip=0.3em]
using UnityEngine;
using System.IO;
...
Ogni \texttt{n} frame, i dati RGB vengono esportati in formato .png e salvati anche in un file .csv, consentendo un'analisi dettagliata dei singoli pixel. Contestualmente, viene generata una mappa di profondità simulata mediante una funzione sinusoidale che varia nel tempo, permettendo una rappresentazione dinamica della distanza.
Essa viene adottata al fine di generare una mappa di profondità controllata, continua e ripetibile. La natura periodica della sinusoide consente di modellare variazioni regolari e progressive nei valori di intensità, simulando efficacemente superfici ondulate, che introducono variazioni continue e non lineari nei dati di profondità, senza l’ausilio di un sensore fisico. Questa scelta permette di verificare con precisione la sensibilità del sistema a variazioni spaziali e temporali della profondità, oltre a garantire la coerenza e la sincronizzazione tra il flusso RGB e quello di profondità. Inoltre, la semplicità computazionale della funzione ne consente l’integrazione in tempo reale, rendendola particolarmente adatta per scenari di test iterativi e riproducibili.

Una sezione specifica dello script è inoltre dedicata alla simulazione di variazioni visive e di profondità localizzate, al fine di testare la sensibilità del sistema nel rilevare modifiche. L’architettura del codice è orientata all’efficienza e alla modularità, risultando adatta a scenari di test e sviluppo per sistemi di visione artificiale basati su dati RGB-D.
...
\begin{lstlisting}[caption=Publisher di Unity in C\#, label=lst:base, lineskip=0.3em]
using UnityEngine;
using System.IO;
...
\begin{tabular}{|c|c|}
\hline
\textbf{Pacchetto} & \textbf{Versione} \\
\hline
opencv-python & 4.8.1 \\
rclpy & 3.3.9 \\
sensor\_msgs & 4.4.0 \\
...
\hline
\end{tabular}
\caption{Dipendenze Python e relative versioni}
\end{table}
...
\texttt{rclpy}          & Libreria client Python per ROS 2 \\
...
\begin{itemize}
    ...
    \item \textbf{robot\_state\_publisher}: questo nodo carica il modello URDF (attraverso il parametro robot\_description), si sottoscrive al topic \texttt{/joint\_states} e infine calcola e pubblica sui topic TF \texttt{(/tf e /tf\_static)} tutte le trasformazioni tra i link del robot, in tempo reale.
\end{itemize}
...
\begin{verbatim}
    ros2 run ros_tcp_endpoint default_server_endpoint --ros-args -p ROS_IP:={IL_TUO_IP}
\end{verbatim}
...
Di seguito alcune acquisizioni dello schermo della scena di Unity in cui è possibile vedere il robot TM5-900 in movimento.

\begin{figure} [H]
    \centering
    \includegraphics[width=1\textwidth]{images/test_movimento_robot_1.PNG}
    \caption{Prima acquisizione del robot in posizione di \texttt{home}}
    \label{fig:test_movimento_robot_1}
\end{figure}

\begin{figure} [H]
    \centering
    \includegraphics[width=1\textwidth]{images/test_movimento_robot_2.PNG}
    \caption{Robot in movimento}
    \label{fig:test_movimento_robot_2}
\end{figure}

\begin{figure} [H]
    \centering
    \includegraphics[width=1\textwidth]{images/test_movimento_robot_3.PNG}
    \caption{Robot in movimento}
    \label{fig:test_movimento_robot_3}
\end{figure}
...
Nel corso di questo progetto è stato realizzato un ambiente integrato in cui un robot manipolatore, gestito tramite ROS 2, interagisce con un mondo virtuale in Unity attraverso il bridge \texttt{ROS-TCP-Endpoint}. Sono stati configurati i topic e i servizi necessari per la trasmissione della traiettoria generata da MoveIt verso il controller di Unity, che riproduce in tempo reale i movimenti del braccio robotico. È stata predisposta una camera virtuale in Unity per monitorare dall’interno della scena sia il robot sia l’oggetto di prova (una maglietta), e si è validato il flusso di dati bidirezionale tra ROS 2 e Unity.
\newline
Durante lo sviluppo si sono acquisite numerose competenze tecniche e operative: l’uso della shell Bash in ambiente Ubuntu per la gestione dei pacchetti ROS 2 e la creazione di script di avvio; la scrittura di nodi in Python per l’interazione con MoveIt2; l’implementazione di componenti C\# in Unity per la ricezione dei comandi di traiettoria e il controllo della camera virtuale; la configurazione del plugin \texttt{ROS-TCP-Endpoint} per garantire la comunicazione senza interruzioni; la personalizzazione di scenari in Gazebo e la modellazione di elementi grafici in Unity; infine, la calibrazione e il allineamento della vista della camera virtuale rispetto al sistema robotico.
\newline
Questo progetto può essere impiegato come piattaforma di test per molteplici applicazioni: modulando le pose del robot, si possono esplorare task diversi come il pick‐and‐place, la saldatura o l’ispezione visiva; integrando algoritmi di computer vision, si potrà abilitare il riconoscimento di oggetti e la guida basata su feedback visivo; grazie al sistema fisico \texttt{Cloth} di Unity per le deformazioni si potrà far reagire realisticamente la maglietta quando il robot la tocca, aprendo la strada a esperimenti di manipolazione di materiali deformabili. Grazie alla flessibilità di ROS 2 e alla potenza grafica di Unity, questa architettura rappresenta un solido punto di partenza per lo sviluppo di scenari sempre più complessi e interattivi.
