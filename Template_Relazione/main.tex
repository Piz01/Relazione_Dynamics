...
In particolare, è stata utilizzata la versione WSL2, che offre prestazioni migliorate rispetto a WSL1 grazie all’integrazione di un vero e proprio kernel Linux. Questa soluzione permette di combinare la potenza e la flessibilità di Ubuntu con la praticità degli strumenti di sviluppo offerti dall’ambiente Windows.L’installazione di Ubuntu è avvenuta tramite WSL (Windows Subsystem for Linux), una funzionalità disponibile su Windows 10 e 11 che consente di eseguire nativamente un ambiente GNU/Linux all’interno di Windows, senza dover ricorrere a una macchina virtuale o a un sistema dual boot.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/ubuntu.png}
    \caption{Logo Ubuntu}
    \label{fig:Ubuntu}
\end{figure}
...
\begin{itemize}
    \item Per seguire passo dopo passo l’installazione, è disponibile una guida video al seguente link: https://www.youtube.com/watch?v=bOwsWabST-0
\end{itemize}

Di seguito, i principali passaggi eseguiti per l’installazione:

\begin{enumerate}
    \item \textbf{Abilitazione delle funzionalità WSL e Virtual Machine Platform}
    \begin{itemize}
...
\begin{enumerate}
\item \textbf{Installazione di Ubuntu 22.04}
    \begin{itemize}
